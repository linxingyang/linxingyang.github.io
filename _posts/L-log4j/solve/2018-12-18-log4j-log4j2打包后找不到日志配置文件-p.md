---
layout: post
permalink: /:year/02fab6daed2b404da56b4df6b59547e0/index
title: log4j-log4j2打包后找不到日志配置文件
categories: [log4j]
tags: [post, log4j, java]
date: 2018-12-18 11:54:35 +8
place: 福州软件园
editdate: 2021-12-12 11:54:35 +8
eidtplace: 福州软件园
...
确认jar包中是存在log4j2.xml的配置文件的。

尝试指定日日志文件路径 -Dlog4j.configurationFile=log4j2.xml 还是不行。

最后发现解决方法如下，在对应的pom.xml中加入如下内容：
```xml
...
* [log4j2找不到配置文件](https://stackoverflow.com/questions/34945438/log4j2-configuration-not-found-when-running-standalone-application-built-by-shad)